...
export const DatabaseStats: React.FC = () => {
  ...
  const [sortColumn, setSortColumn] = useState<'tableName' | 'recordCount'>('recordCount')
    ...
    const fetchWorldEntities = async () => {
      try {
        ...
        const filteredEntities = response.data.filter(entity => 
          !entity.is_hidden && 
          entity.table_name.indexOf("_has_") === -1
        )
        ...
        const initialStats = filteredEntities.map(entity => {
          const isSystem = entity.table_name.startsWith('_') ||
            ['world', 'action', 'user_account', 'usergroup', 'smd', 'oauth_connect', 'oauth_token'].includes(entity.table_name)
    ...
    const fetchTableCounts = async () => {
      ...
      for (const entity of worldEntities) {
        try {
          ...
          setTableStats(prevStats => {
            return prevStats.map(stat => {
              if (stat.tableName === entity.table_name) {
                return {
                  ...stat,
                  recordCount: result && result.length > 0 ? result[0].attributes.count : 0,
                }
              }
              return stat
            })
          ...
          console.error(`Failed to fetch count for ${entity.table_name}:`, error)
        ...
        {isLoading ? (
          ...
          <div className="rounded-md border">
            <UITable>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-[50%] cursor-pointer" onClick={() => toggleSort('tableName')}>
                  ...
                  <TableHead className="text-right cursor-pointer" onClick={() => toggleSort('recordCount')}>
                  ...
                  <TableRow key={stat.tableName}>
                    <TableCell className="font-medium">
                      <div className="flex items-center">
                        ...
                        <span className="capitalize">{stat.tableName.replace(/_/g, ' ')}</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-right">{stat.recordCount.toLocaleString()}</TableCell>

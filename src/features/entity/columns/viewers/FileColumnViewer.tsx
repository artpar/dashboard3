    ...
    return (
      <>
        <div className={cn("flex items-center space-x-2", className)}>
          <div
            className="h-8 w-8 bg-muted flex items-center justify-center rounded overflow-hidden"
          ...
          <span className="truncate max-w-xs">{file.name || 'Image'}</span>
        </div>
    ...
    <div className={cn("space-y-1", className)}>
      {fileList.map((file, index) => (
        <div key={index} className="flex items-center space-x-2">
          ...
          <span className="truncate max-w-xs text-sm">{file.name || `File ${index + 1}`}</span>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => handleViewFile(file)}

    ...
    <div className={cn('space-y-2', className)}>
      <div className='flex items-center justify-between space-x-2'>
        ...
        >
          ...
          {uploading
            ? 'Uploading...'
            : `${allowMultiple ? 'Add Files' : 'Choose File'}`}
        </Button>
      ...
      {files.length > 0 && (
        <div className='mt-2 space-y-2'>
          {files.map((file, index) => (
            <div
              key={index}
              className='bg-muted flex items-center space-x-2 rounded-md p-2'
            >
              {file.file && isImageFile(file) ? (
                <div className='bg-background flex h-10 w-10 items-center justify-center overflow-hidden rounded'>
                  <img
                    src={file.file}

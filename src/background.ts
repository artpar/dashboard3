...
const setTokenExpiryTimeout = async () => {
    ...
    console.log('setting user token timeout for ' + expireTokenTimestamp + ' after ' + parseInt('' + delayTime) + ' ' + resolution)
    tokenTimoutReference = setTimeout(async () => {
        console.log('token timed out, removing user ref: ' + JSON.stringify(daptinUserAuth))
        localStorage.removeItem('daptin')
        // await frontAgent.reloadToken()
        daptinUserAuth = {
            user: null, token: null, customer: null, creator: null,
        }
    }, timeoutDuration)
}
...
async function fetchAllWorksheets(taskId) {
    ...
    while (hasMore) {
        ...
        allWorksheets = allWorksheets.concat(response.data.map((e) => JSON.parse(e.row_data)))
        ...
        try {
            ...
            switch (request.type) {
                ...
                case 'signInWithEmailOtp':
                    ...
                    try {
                        signinWithEmailOtpResponse = await daptinClient.actionManager.doAction('user_account', 'signin_100xbot_email', {
                            token: btoa(JSON.stringify({email: request.email, otp: request.otp})),
                        })
                    } catch (e) {
                        if (e.response.data) {
                    ...
                    if (signinWithEmailOtpResponseElement.ResponseType === 'client.notify') {
                        reject({
                            error: signinWithEmailOtpResponseElement.Attributes.message, status: 'error',
                        })
                        return
                    }
                    const newUserToken1 = signinWithEmailOtpResponseElement.Attributes.value
                    let newUserObject1 = JSON.parse(atob(newUserToken1.split('.')[1]))
                    let customer1 = signinWithEmailOtpResponse.filter((e) => e.ResponseType === 'customer')[0].Attributes[0]
                    let credit1 = signinWithEmailOtpResponse.filter((e) => e.ResponseType === 'credit')[0].Attributes[0]
                    let credits = signinWithEmailOtpResponse.filter((e) => e?.ResponseType === 'client.notify')?.[0]?.Attributes || {};

                    const creatorProfile = await daptinClient.jsonApi.findAll('creator', {
                        customer_id: customer1.reference_id, customerName: "creator_id", included_relations: '',
                    })
                    ...
                    resolve({
                        data: {...daptinUserAuth, credits}, error: null, status: 'success',
                    })
                    break
                case 'signOut':
                    localStorage.removeItem('DAPTIN')
                ...
                case 'createWaitlistEntry':
                    const waitlistResponse = await daptinClient.jsonApi.create('waitlist', {
                        email: request.payload.email, name: request.payload.name, industry: request.payload.industry,
                    })
                    ...
                    break
                case 'createDocument':
                    const newDocumentResponse = await daptinClient.jsonApi.create('document', {
                        document_name: 'new document - ' + new Date().getTime() + '.crdt.json',
                    ...
                    break
                case 'getDocument':
                    const documentResponse = await daptinClient.jsonApi.find('document', request.reference_id, {
                    ...
                    break
                case 'getAllMemories':
                    const memoryListResponse = await daptinClient.jsonApi.findAll('memory', {
                        "sort": "display_order",
                    })
                ...
                case 'getAllMemoriesByQuery':
                    let allMemoriesByQuery = await daptinClient.jsonApi.findAll('memory', {
                        'page[size]': 50, query: JSON.stringify([{
                        ...
                        },]), "sort": "display_order", "included_relations": "creator_id,workgroup_id"
                    })
                    resolve(allMemoriesByQuery.data)
                    break
                case 'getMemory':
                    const memoryResponse = await daptinClient.jsonApi.findAll('memory', {})
                    ...
                    break
                case 'getMemoryById':
                    const memoryOneResponse = await daptinClient.jsonApi.find('memory', request.reference_id, {
                ...
                case 'getMemoryByCreatorId':
                    const memoryCreatorResponse = await daptinClient.jsonApi.findAll('memory', {
                        creator_id: request.reference_id, creatorName: "memory_id", query: JSON.stringify([{
                            column: 'show_on_sidepanel', operator: 'eq', value: '1',
                        }]), included_relations: 'article_id',
                    })
                    if (memoryCreatorResponse.errors && memoryCreatorResponse.errors.length) {
                ...
                case 'create_reply':
                    try {
                        ...
                        if (!content || !parent_type || !parent_id) {
                            ...
                            reject({error: 'Missing required fields: content, parent_type, and parent_id are required'})
                            return
                        }
                        ...
                        if (!['memory', 'article', 'reply'].includes(parent_type)) {
                            ...
                            reject({error: 'Invalid parent_type. Must be one of: memory, article, reply'})
                            return
                        }

                        // Create the reply using the appropriate action
                        const replyResponse = await daptinClient.actionManager.doAction('reply', 'create_reply', {
                            content: content, parent_type: parent_type, parent_id: parent_id,
                        },)

                        // Check for notification responses
                        const notificationResponse = replyResponse.find(res => res.ResponseType === 'client.notify')
                        ...
                        const createdReply = replyResponse.find(res => res.ResponseType === 'reply')?.Attributes
                ...
                case 'getReviewsByMemoryId':
                    const reviewResponse = await daptinClient.jsonApi.findAll('review', {
                        memory_id: request.reference_id, memoryName: "review_id", included_relations: 'customer_id',
                    })
                ...
                case 'getCreatorBySlug':
                    const creatorSlugResponse = await daptinClient.jsonApi.findAll('creator', {
                        query: JSON.stringify([{
                            column: 'creator_slug', operator: 'eq', value: request.slug,
                        },]),
                    })
                ...
                case 'getCreatorInfoById':
                    const creatorResponse = await daptinClient.jsonApi.find('creator', request.reference_id, {
                        included_relations: 'article_id',
                    })
                ...
                case 'getCreatorInfoByCustomerId':
                    const getCreatorByCustomerId = await daptinClient.jsonApi.findAll('creator', {
                        customer_id: request.reference_id,
                        customerName: "creator_id",
                        included_relations: 'creator_image',
                    })
                    if (getCreatorByCustomerId.errors && getCreatorByCustomerId.errors.length) {
                ...
                case "updateCreatorInfo":
                    const updatedCreatorInfo = await daptinClient.actionManager.doAction('customer', 'update_creator', request.payload)
                ...
                case "addCreatorProfile":
                    const addedCreatorInfo = await daptinClient.actionManager.doAction('customer', 'addCreator', request.payload)
                    ...
                    localStorage.setItem('DAPTIN', JSON.stringify(daptinUserAuth))
                    resolve(addedCreatorInfo.filter(e => e.ResponseType === "creator")[0].Attributes)
                    break
                case "updateCustomerInfo":
                    const updatedCustomerInfo = await daptinClient.actionManager.doAction('customer', 'update_customer', request.payload)
                    ...
                    localStorage.setItem('DAPTIN', JSON.stringify(daptinUserAuth))
                    resolve(updatedCustomerInfo[updatedCustomerInfo.length - 1].Attributes)
                    break
                case 'getArticlesByCreatorId':
                    const getArticlesByCreatorId = await daptinClient.jsonApi.findAll('article', {
                        creator_id: request.reference_id, creatorName: "article_id",
                    })
                    if (getArticlesByCreatorId.errors && getArticlesByCreatorId.errors.length) {
                ...
                case 'getArticlesByWorkgroupId':
                    const creatorArticleResponse = await daptinClient.jsonApi.findAll('article', {
                        "workgroup_id": request.reference_id, "workgroupName": "article_id",
                    })
                    if (creatorArticleResponse.errors && creatorArticleResponse.errors.length) {
                ...
                case 'getMemoryByTitle':
                    ...
                    const taskByMemory = await daptinClient.jsonApi.findAll('memory', {
                        query: JSON.stringify([{
                            column: 'title', operator: 'eq', value: request.title,
                        },]), included_relations: 'creator_id,article_id,workgroup_id',
                    })
                ...
                case 'getMemoryByQuery':
                    ...
                    const taskByQuery = await daptinClient.jsonApi.findAll('memory', {
                        query: JSON.stringify([{
                            column: 'title', operator: 'iLike', value: request.query,
                        },]), included_relations: 'creator_id,article_id,workgroup_id',
                    })
                    ...
                    break
                case 'getTask':
                    const taskResponse = await daptinClient.jsonApi.find('rpatask', request.reference_id)
                ...
                case 'getTaskWorksheet':
                    const taskWorksheetResponse = await daptinClient.jsonApi.findAll('worksheet', {
                        rpatask_id: request.task_id,
                        rpataskName: "worksheet_id",
                        'page[size]': '500',
                        sort: 'created_at',
                    })
                    if (taskWorksheetResponse.errors && taskWorksheetResponse.errors.length) {
                ...
                case 'getWorkgroups':
                    let workgroups = await daptinClient.jsonApi.findAll('workgroup', {
                        'page[size]': 100, query: JSON.stringify([{
                            column: 'show_on_sidepanel', operator: 'eq', value: '1',
                        },])
                    })
                ...
                case 'getWorkgroupByName':
                    let workgroupByName = await daptinClient.jsonApi.findAll('workgroup', {
                        'page[size]': 100, query: JSON.stringify([{
                            column: 'name', operator: 'ilike', value: request.name,
                        }, {
                            column: 'show_on_sidepanel', operator: 'eq', value: '1',
                        },])
                    })
                ...
                case 'getWorkgroupByQuery':
                    let workgroupByQuery = await daptinClient.jsonApi.findAll('workgroup', {
                        'page[size]': 100, query: JSON.stringify([{
                            column: 'name', operator: 'ilike', value: '%' + request.query + '%',
                        }, {
                            column: 'show_on_sidepanel', operator: 'eq', value: '1',
                        },])
                    })
                ...
                case 'getWorkgroupsByType':
                    let workgroupByType = await daptinClient.jsonApi.findAll('workgroup', {
                        'page[size]': request.limit, query: JSON.stringify([{
                            column: 'group_type', operator: 'eq', value: request.groupType,
                        }, {
                            column: 'show_on_sidepanel', operator: 'eq', value: '1',
                        },])
                    })
                ...
                case 'getUserWorkgroups':
                    let userWorkgroups = await daptinClient.jsonApi.findAll('workgroup', {})
                ...
                case 'checkFavouriteMemory':
                    ...
                    const isFavourite = allFavourites.data.filter((m) => m.id === request.memory_id).length > 0;
                    resolve({
                ...
                case 'getMemoriesForWorkgroup':
                    let memoryFromWg = await daptinClient.jsonApi
                        ...
                        .all('memory_id')
                        .get({
                            query: request.title ? JSON.stringify([{
                                column: 'title', operator: 'iLike', value: request.title,
                            }, {
                                column: 'show_on_sidepanel', operator: 'eq', value: '1',
                            }]) : JSON.stringify([{
                                column: 'show_on_sidepanel', operator: 'eq', value: '1',
                ...
                case 'getArticleBySlug':
                    const memoryArticle = await daptinClient.jsonApi.findAll('article', {
                        query: JSON.stringify([{
                            column: 'slug', operator: 'eq', value: request.slug,
                ...
                case 'getFeaturedArticle':
                    const featuredArticle = await daptinClient.jsonApi.findAll('article', {
                        "sort": "-created_at", "included_relations": 'creator_id,workgroup_id',
                    })
                    resolve({
                ...
                case 'sendInviteToEmails':
                    const response = await daptinClient.actionManager.doAction('user_account', 'send_custom_invite_email', {
                        emails: request.emails, referral_code: request.referral_code,
                    })

                    const inviteStatus = response.find((e) => e.ResponseType === 'client.notify')?.Attributes?.emailStatus || 'Sorry, Could not send the invites!'
                    ...
                    if (inviteStatus === 'messages sent') {
                        ...
                        resolve({
                            success: false, error: inviteStatus,
                        })
                    }

                    break
                case 'getAllTask':
                    const taskListResponse = await daptinClient.jsonApi.findAll('rpatask', {
                        sort: '-created_at', query: JSON.stringify([{
                            column: 'root_rpatask', operator: 'is null', value: 'null',
                        },]),
                    })
                    ...
                    resolve(taskListResponse.data.filter(e => e.prompt.indexOf("Always running task") === -1))

                    break
                case 'getStep':
                    const stepResponse = await daptinClient.jsonApi.find('rpastep', request.reference_id)
                ...
                case 'getStepByTaskId':
                    const stepListResponse = await daptinClient.jsonApi.findAll('rpastep', {
                        rpatask_id: request.task_id, rpataskName: "rpastep_id",
                    })
                    ...
                    break
                case 'updateDocument':
                    const uDocumentResponse = await daptinClient.jsonApi.update('document', {
                        ...request.document,
                    })
                ...
                case 'signInWithEmail':
                    const signinWithEmailResponse1 = await daptinClient.actionManager.doAction('user_account', 'signup_100xbot_email', {
                        email: request.email,
                    })
                    console.log('Initiate email otp for ', request.email, signinWithEmailResponse1)
                ...
                case 'signInWithEmailWithReferral':
                    const signinWithEmailResponse = await daptinClient.actionManager.doAction('user_account', 'signup_100xbot_email_referral', {
                        email: request.email, referral_code: request.referral_code,
                    })
                ...
                case 'getMemoryReplies':
                    try {
                        ...
                        if (memoryRepliesResponse.errors && memoryRepliesResponse.errors.length) {
                        ...
                        reject({error: 'Failed to fetch memory replies: ' + error.message})
                    }
                    break
                case 'create_upvote':
                    try {
                        ...
                        if (!vote_type || !parent_type || !parent_id) {
                            ...
                            reject({error: 'Missing required fields: vote_type, parent_type, and parent_id are required'});
                            return;
                        }
                        ...
                        if (!['upvote', 'downvote'].includes(vote_type)) {
                            ...
                            reject({error: 'Invalid vote_type. Must be one of: upvote, downvote'});
                            return;
                        }

                        // Create the upvote using the appropriate action
                        const upvoteResponse = await daptinClient.actionManager.doAction('upvote', 'create_upvote', {
                            vote_type: vote_type, parent_type: parent_type, parent_id: parent_id,
                        },);

                        // Check for notification responses
                        const notificationResponse = upvoteResponse.find(res => res.ResponseType === 'client.notify');
                        if (notificationResponse) {
                            resolve({
                                success: true,
                        ...
                        const createdUpvote = upvoteResponse.find(res => res.ResponseType === 'upvote')?.Attributes;
                ...
                case 'check_user_vote':
                    try {
                        ...
                        if (!parent_type || !parent_id) {
                            ...
                            reject({error: 'Missing required fields: parent_type and parent_id are required'});
                            return;
                        }
                        ...
                        const checkVoteResponse = await daptinClient.actionManager.doAction('upvote', 'check_user_vote', {
                            parent_type: parent_type, parent_id: parent_id,
                        },);

                        // Extract the vote information from the response
                        const voteInfo = checkVoteResponse.find(res => res.ResponseType === 'client.notify')?.Attributes;
                        ...
                        resolve({
                            has_voted: voteInfo.has_voted, vote_type: voteInfo.vote_type
                        });
                    } catch (error) {
                ...
                case 'get_vote_count':
                    try {
                        ...
                        const voteCountResponse = await daptinClient.actionManager.doAction('upvote', 'get_vote_count', {
                            parent_type: parent_type, parent_id: parent_id,
                        },);

                        // Extract the vote count information from the response
                        const voteCountInfo = voteCountResponse.find(res => res.ResponseType === 'client.notify')?.Attributes;
                ...
                case 'getArticleReplies':
                    try {
                        ...
                        if (articleRepliesResponse.errors && articleRepliesResponse.errors.length) {
                        ...
                        reject({error: 'Failed to fetch article replies: ' + error.message})
                    }
                    break
                case 'getReplyReplies':
                    try {
                        ...
                        const replyRepliesResponse = await daptinClient.jsonApi
                            ...
                            .get({
                                sort: '-created_at',
                                included_relations: 'customer_id,reply_id,child_reply_id,parent_reply_id',
                                'page[size]': request.limit || 50
                            })

                        if (replyRepliesResponse.errors && replyRepliesResponse.errors.length) {
                ...
                case 'deleteArticle':
                    try {
                        ...
                        const deleteArticleResponse = await daptinClient.actionManager.doAction('article', 'delete_article', {
                            article_id: request.articleId
                        });

                        // Check for success notification
                        const successResponse = deleteArticleResponse.find(res => res.ResponseType === 'client.notify' && res.Attributes.status === 'success');

                        if (!successResponse) {
                            // Check for error notification
                            const errorResponse = deleteArticleResponse.find(res => res.ResponseType === 'client.notify' && res.Attributes.status === 'error');
                        ...
                        resolve({success: true, message: successResponse.Attributes.message});
                    } catch (error) {
